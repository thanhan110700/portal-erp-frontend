  ...
  if (!interactions.length) {
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center border rounded-xl bg-card border-dashed">
        ...
        <p className="text-sm font-medium">Chưa có lịch sử tương tác nào</p>
        <p className="text-xs text-muted-foreground mt-1">
          Bấm Thêm tương tác để ghi nhận lịch sử làm việc với khách hàng này.
        </p>
      </div>
    )
  }
        ...
        >
          ...
          <div className="flex-1 space-y-1">
            <div className="flex items-center justify-between">
              <h4 className="font-semibold text-sm">{interaction.type || "Tương tác"}</h4>
            ...
            {interaction.user && (
              <p className="text-xs text-muted-foreground pt-1">
                Ghi nhận bởi: <span className="font-medium">{interaction.user.full_name}</span>
              </p>
            )}
          </div>
        </div>
